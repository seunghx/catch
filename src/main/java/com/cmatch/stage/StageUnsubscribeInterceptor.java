...
/**
 * 
 * InboundChannel에 등록될 것을 고려하여 작성된 {@link ChannelInterceptor} 구현.
 * 
 ...
 * RabbitMQ를 이용하여 클러스터 내의 모든 서버 노드에서 통신 가능하게 되어 있다. 
 * 
 ...
 * 기존 subscription id를 서버에 전달하는 방식으로 동작한다. 
 * 
...
public class StageUnsubscribeInterceptor extends ChannelInterceptorAdapter {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if(command == StompCommand.UNSUBSCRIBE) {
            ...
            log.debug("Received stage unsubscription message.");
            ...
            if(channel instanceof AbstractSubscribableChannel) {
                ...
                log.debug("Stage unsubscribe message is processed successfully.");
